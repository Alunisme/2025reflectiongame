...
let screenColor = 220; // 新增變數，控制螢幕顏色
...
function draw() {
  background(screenColor); // 使用 screenColor 作為背景顏色
...
function mousePressed() {
  if (!gameStarted) {
    ...
    if (d < targetSize / 2) {
      score++;
      screenColor = color(0, 255, 0); // 成功觸摸，螢幕亮綠燈
      resetTarget();
    } else {
      score--;
      screenColor = color(255, 0, 0); // 未觸摸到，螢幕亮紅燈
    }
  }
}
...
function startGame() {
  ...
  if (intervalId) {
    ...
    intervalId = null; // 確保 intervalId 被重置
  }
